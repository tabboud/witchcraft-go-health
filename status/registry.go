...
//
// This is almost the same as the reporter.HealthReporter interface, but that is scoped to a HealthComponent instead
type HealthCheckRegistry interface {
	HealthCheckSource
	Register(CheckType, HealthCheckSource)
	Unregister(CheckType)
	Get(key CheckType) (HealthCheckSource, bool)
}

type HealthCheckSourceRegistry struct {
	mutex    sync.RWMutex
	registry map[CheckType]HealthCheckSource
}
...
func NewHealthCheckSourceRegistry() *HealthCheckSourceRegistry {
	return &HealthCheckSourceRegistry{
		mutex:    sync.RWMutex{},
		registry: make(map[CheckType]HealthCheckSource),
	}
}
...
func (h *HealthCheckSourceRegistry) HealthStatus(ctx context.Context) health.HealthStatus {
	...
	sources := make([]HealthCheckSource, 0, len(h.registry))
	for _, source := range h.registry {
		sources = append(sources, source)
	}

	return NewCombinedHealthCheckSource(sources...).HealthStatus(ctx)
}

func (h *HealthCheckSourceRegistry) Register(key CheckType, source HealthCheckSource) {
	h.mutex.Lock()
	defer h.mutex.Unlock()
	h.registry[key] = source
}

func (h *HealthCheckSourceRegistry) Unregister(key CheckType) {
	h.mutex.Lock()
	defer h.mutex.Unlock()
	delete(h.registry, key)
}

func (h *HealthCheckSourceRegistry) Get(key CheckType) (HealthCheckSource, bool) {
	h.mutex.Lock()
	defer h.mutex.Unlock()
